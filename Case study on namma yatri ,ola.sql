...
select * from trips_details4;
Rename table trip_details to trips_details;
...
order by cnt desc ;
#or
select p.method from payment as p inner join (select distinct faremethod,count(distinct tripid) as cnt from trips
group by faremethod
order by cnt desc) as T on p.id=t.faremethod;
...
# Mahadevapura -- fair and trips & Cancellation
# less cancel--- CV raman road
